...
1. 实现标准答案数据模型与 API：每条标准答案绑定一个已确认 `ExamRegion`，保存参考答案、满分和评分规则。
2. 新增标准答案工作台：教师按题区录入答案、满分和评分点，未填写题区需要明确提示。
3. 扩展学生答卷处理任务：有标准答案时生成评分草稿，无标准答案时只生成 OCR draft 并标记待补答案。
4. 扩展教师复核页：显示标准答案、评分规则、OCR draft、建议分和建议评语，教师最终确认。
...
- 采用模板驱动流程，不做无模板整卷盲识别。
- 第一版标准答案采用教师手动录入，并且必须绑定已确认的 `ExamRegion`。
