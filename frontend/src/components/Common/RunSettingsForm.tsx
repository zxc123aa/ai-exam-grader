...
/**
 * 批改设置表单（判题提供者/模型、复核阈值、答卷级与题目级并发）。
...
export function RunSettingsForm() {
  ...
  const totalConcurrency = Math.min(
    32,
    Number(parallelSubmissions || 1) * Number(concurrencyPerSubmission || 1),
  )
    ...
    <div className="grid gap-4">
      ...
      <div className="grid gap-2">
        ...
        <Input
          ...
          min="1"
          max="8"
          value={parallelSubmissions}
          onChange={(event) =>
            patchRunSettings({ parallelSubmissions: event.target.value })
          }
        />
        <p className="text-muted-foreground text-xs">
          同一时间处理多少份学生答卷，默认 8 份
        </p>
      </div>
      <div className="grid gap-2">
        <Label>每份答卷并行题数</Label>
        <Input
          type="number"
          min="1"
          max="8"
          value={concurrencyPerSubmission}
          onChange={(event) =>
            patchRunSettings({ concurrencyPerSubmission: event.target.value })
          }
        />
        <p className="text-muted-foreground text-xs">
          一份卷子内部同时处理多少道题，默认 4 题
        </p>
      </div>
      <div className="border-t pt-3 text-muted-foreground text-xs">
        当前最多同时处理 {totalConcurrency}{" "}
        个题目任务，系统会在服务繁忙时自动降速
      </div>
      <Button
        variant="ghost"
