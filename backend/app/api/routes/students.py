...
def _snap_extract_multi(
    ...
    prompt = (
        ...
        "150 字以内；2) 学生的手写作答——你是转录员，只誊写不评判："
        "必须原样照抄，写错也不许纠正，禁止把学生的错误答案改成正确答案，"
        "禁止约分、化简、分数小数互化等任何形式的「顺手改写」"
        "（学生写 5/7 就抄 5/7，即使正确答案是 5/6）；"
        "手写数字逐位核对，两位数别读成一位数，分数线上下别读反；"
        "数字后面的单位字一个不漏（万、千、百、个、本、元、千克、平方分米等，"
        "「30 万本」不能丢成「30 本」）；"
        "看不清写「无法辨认」，拿不准就标存疑，不要猜；"
...
def _snap_solve_and_grade_all(
    ...
    prompt = (
        ...
        "选择题/判断题/填空题只判结果对错（对=满分，错=0 分，不设步骤分不扣过程分）；"
        "多空题/多小题必须按空逐一对应核对，答案内容对但写错了位置"
        "（如两空对调）按错判，不算全对；"
...
def _snap_grade_one(
    *,
    ...
    cache_key = hashlib.sha256(
        f"v4|{question_text}|{student_answer}|{max_score}".encode()
    ).hexdigest()
    ...
    prompt = (
        ...
        "1) 选择题/判断题/填空题只判结果对错：对给满分，错给 0 分，不设步骤分、不扣过程分；"
        "多空题/多小题必须按空逐一对应核对，答案内容对但写错了位置"
        "（如两空对调）按错判，不算全对；"
